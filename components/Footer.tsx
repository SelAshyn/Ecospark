...
export default function Footer() {
  return (
    <footer className="relative z-10 bg-[rgba(4,13,7,0.95)] border-t border-[rgba(82,183,136,0.1)] px-6 py-12">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 flex-wrap">
          <div className="flex items-center gap-2">
            <div className="group-hover:-rotate-[8deg] group-hover:scale-[1.08] transition-transform duration-300 drop-shadow-[0_0_8px_rgba(116,198,157,0.5)]">
              <EcosparkLogo size={32} />
            </div>
            <span className="font-serif text-[1.1rem] font-bold text-white">
              Eco<span className="text-[#95d5b2]">spark</span>
            </span>
          </div>
          <div className="flex gap-8 flex-wrap">
            {[['About', '/about'], ['Contact', '/contact'], ['Privacy', '/privacy']].map(([label, href]) => (
              <Link
                key={href}
                href={href}
                className="text-[0.85rem] font-medium text-[rgba(183,228,199,0.4)] hover:text-[#74c69d] transition-colors duration-200 no-underline tracking-[0.03em]"
              >
                {label}
              </Link>
            ))}
        ...
        <p className="text-[0.8rem] text-[rgba(183,228,199,0.25)] tracking-[0.04em] text-center mt-8 pt-8 border-t border-[rgba(82,183,136,0.08)]">
          © 2026 Ecospark · Where action sparks sustainability.
        </p>
      </div>
    </footer>
  );
